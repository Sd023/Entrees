...
class CartRepo(var context: Context) {
    ...
    fun changeStatusInTable(isDataInserted: Boolean, tableName: String,seats: String){
        try {
            val master = FirebaseDatabase.getInstance().getReference("hotels").child(hotel).child(hotelBranch)
            ...
            if(tableName.isNotEmpty() && seats.isNotEmpty()){
                if(isDataInserted){
                    ref.child(tableName).child("isStatus").setValue("ORD").addOnCompleteListener{task ->
